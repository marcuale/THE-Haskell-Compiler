...
class Parser:
    ...
    def p_function_type(self, p):
        ...
        '''
        p[0] = Type(Type(TypeConstrVar("->"), p[1]), p[3])
    ...
    def p_funcdef(self, p):
        ...
        '''
        if p[1][0] != p[3][0][0]:
            raise SyntaxError("Different name in type and body")
            

        p[0] = FuncDef(p[1][1], p[3][0], p[3][1])


    def p_typelessfuncdef(self, p):
        '''
        funcdef : funcbody
        '''
        p[0] = FuncDef(None, p[1][0], p[1][1])
...
if __name__ == "__main__":
    m = Parser()
    m.build()
    m.parse_print(open(sys.argv[1]).read())
