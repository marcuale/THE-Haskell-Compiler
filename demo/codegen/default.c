...
THCObject * Default_unsafePrint_obj;
THCObject * Default_seq_obj;
...
void unsafePrint(THCObject * this, THCList * args){
    evaluate(this, args->head->contents.n.value);
    int i = args->head->contents.n.value->contents.i;
    printf("%d\n", i);
    
    THCObject * cont = this->contents.f.cont;
    callCont(cont, args->head->contents.n.value);
}
...
void seq(THCObject * this, THCList * args){
    evaluate(this, args->head->contents.n.next->contents.n.value);
...
void initDefaults(){
    ...
    Default_unsafePrint_obj = constructFunction(unsafePrint, 1);
    Default_seq_obj = constructFunction(seq, 2);
